...
import pandas as pd
from etl.core.products import unify


def run (input_es: str | Path, input_uk: str | Path, out_path: str | Path) -> dict:
    
    """
    Step to unify Spanish and UK products:
    - Validates required columns and product code format
    - Standardizes column names to a common schema
    - Normalizes unit prices to numeric
    - Outer-merges on product_code 
